			...
			for (int j = 0; j < GRID_SIZE; j++)
			{
				...
				{ // côtés
					if (is_valid(neighbours[k]))
					{
						...
						if (get_tile(*grid, neighbours[k]).current_type == TREE && random % M1_C_PROBA_TREE_BURN == 0 ||
							get_tile(*grid, neighbours[k]).current_type == GRASS && random % M1_C_PROBA_GRASS_BURN == 0)
						{
							copy[neighbours[k].x][neighbours[k].y].current_type = FIRE;
							copy[neighbours[k].x][neighbours[k].y].state = 0;
						}
				...
				{ // diag
					if (is_valid(neighbours[k]))
					{
						...
						if (get_tile(*grid, neighbours[k]).current_type == TREE && random % M1_D_PROBA_TREE_BURN == 0 ||
							get_tile(*grid, neighbours[k]).current_type == GRASS && random % M1_D_PROBA_GRASS_BURN == 0)
						{
							copy[neighbours[k].x][neighbours[k].y].current_type = FIRE;
							copy[neighbours[k].x][neighbours[k].y].state = 0;
						}
